...
class MouseMazeEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, size=8, slippery=False, epsilon=0.1):
        ...
        self.hole_positions = self._generate_holes()
        self.lastaction = None
        self.path_length = 0
        self.optimal_path_length = 2 * size  # Adjust based on grid size
    ...
    def _generate_holes(self):
        ...
        num_holes = (self.size * self.size) // 10  # Reduce number of holes
        hole_positions = set()

        while len(hole_positions) < num_holes:
            row = np_random.integers(0, self.size)
            col = np_random.integers(0, self.size)
            if (row, col) not in [self.start_position, self.goal_position]:
                hole_positions.add((row, col))

        return list(hole_positions)

    def reset(self, seed=None, options=None):
        super().reset(seed=seed)
        self.state = self.start_position
        self.np_random, _ = seeding.np_random(seed)
        self.hole_positions = self._generate_holes()  # Regenerate holes
        self.lastaction = None
        self.path_length = 0
